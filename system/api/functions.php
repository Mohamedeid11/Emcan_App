            ...
            <table align="center" border="1" cellpadding="0" cellspacing="0" width="600px" style="margin:15px;padding:15px;border-collapse: separate; border-spacing: 5px 5px; #000;" bgcolor="#FFFFFF">
                <tr>
                    <td align="center" style="padding:5px;">
                        <a href="http://berry74.com/" target="_blank">
                            <img src="http://berry74.com/system/api/berry-logo.jpg" alt="Logo" style="width:186px;border:0;"/>
                        </a>
                    </td>
                </tr>
                <tr>
                        ...
                        <table cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:15px;text-align:right;direction:rtl;">
                            <tr>
                                <td style="padding:0; font-family: Avenir, sans-serif; font-size: 16px;">
                                    ...
                                    <h2 style="text-align:center;color:#31A4F1;font-family:tahoma;">ملخص طلبك من تطبيق بيري فرع ' . get_branch_name($branch_id) . '</h2>

                                  <br/>
                                  <table style="border:1px solid #eee;padding: 5px;width: 100%;text-align: center;margin-bottom: 30px;" class="table table-striped">
	                                  <thead>
                                            <th style="width: 40%;font-size: 14px;font-weight: bold;font-family: tahoma;">إسم الصنف</th>
                                            <th style="width: 40%;font-size: 14px;font-weight: bold;font-family: tahoma;">الكمية</th>
                                            <th style="width: 20%;font-size: 14px;font-weight: bold;font-family: tahoma;">الإجمالي</th>
	                                 </thead>
	                                  <tbody>';
    ...
    $message .= '</tbody></table>
                                  ...
                                  <table style="border:1px solid #eee;padding: 5px;width: 100%%; text-align: center;margin-bottom: 30px;" class="table table-striped">
	                                  ...
	                                  <tbody>
	                                  	<tr style="border: 1px solid black;">
	                                  		...
	                                  		<td style="font-size: 12px;padding-top: 10px;border-top: 1px solid #000;">' . get_region_name($client_addresses_row_select['region']) . '</td>
                                  ...
                                  <table style="border:1px solid #eee;padding: 5px;width: 100%%; text-align: center;margin-bottom: 30px;" class="table table-striped">
	                                  <thead>
	                                	  ...
	                                	  <th style="width: 15%;font-size: 14px;font-weight: bold;font-family: tahoma;">الخصم</th>
	                                	  <th style="width: 15%;font-size: 14px;font-weight: bold;font-family: tahoma;">بعد الخصم</th>
	                                	  <th style="width: 20%;font-size: 14px;font-weight: bold;font-family: tahoma;">القيمة المُضافة</th>
	                                	  <th style="width: 20%;font-size: 14px;font-weight: bold;font-family: tahoma;">تكلفة التوصيل</th>
	                                	  <th style="width: 15%;font-size: 14px;font-weight: bold;font-family: tahoma;">الإجمالي</th>
	                                  </thead>
	                                  <tbody>
	                                  	<tr style="border: 1px solid black;">
                            ...
                            <tr>
                                <td width="260" valign="top" style="padding: 0 0 15px 0;">
                                    <table cellpadding="0" cellspacing="0" width="100%%">
                                        <tr>
                                            <td align="center">
                                                <a href="tel:37378322" target="_blank">
                                                    <img src="http://berry74.com/system/api/phone-icon.jpeg" alt="اتصل بنا"
                                                         style="display: block;"/>
                                                </a>
                                            </td>
                                        </tr>
                                ...
                                <td width="260" valign="top">
                                    <table cellpadding="0" cellspacing="0" width="100%%" >
                                        <tr>
                                            <td align="center">
                                                <a href="mailto:emcansolutions@gmail.com">
                                                    <img src="http://berry74.com/system/api/email-icon.jpeg" alt="Email us"
                                                         style="display: block;"/>
                                                </a>
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            <td>
                                                <a href="#" target="_blank">
                                                    <img src="http://berry74.com/system/api/facebook-icon.jpeg" alt="Facebook" width="50" height="50" style="display: block;"/>
                                                </a>
                                            </td>
                                            <td style="font-size: 0; line-height: 0;" width="20">&nbsp;</td>
                                            <td>
                                                <a href="#" target="_blank">
                                                    <img src="http://berry74.com/system/api/instagram-icon.jpeg" alt="Instagram" width="50" height="50"style="display: block;"/>
                                                </a>
                                            </td>
                                            <td style="font-size: 0; line-height: 0;" width="20">&nbsp;</td>
                                            <td>
                                                <a href="#" target="_blank">
                                                    <img src="http://berry74.com/system/api/twitter-icon.jpeg" alt="Twitter" width="50" height="50"style="display: block;"/>
                                                </a>
                                            </td>
                                        </tr>
                                    </table>
...
function resume_evaluate($washer_id) {

    $sql = "SELECT * from `comments` WHERE `washer_id`='$washer_id'";
    ...
    if (mysql_num_rows($result) > 0) {
        ...
        $rates = $sum / $count_evaluate;
    }
    return $rates;
}

function resume_viewed($washer_id) {
...
function get_projects($portfolio_id) {

    $data = array();
//    if ($client_id > 0) {
//        $query = mysql_query(" UPDATE comments SET viewed = viewed + 1 WHERE washer_id = '{$washer_id}' ");
//    }
    $result = mysql_query("SELECT * FROM `sub_portfolio` WHERE `portfolio_id`='$portfolio_id' and `display`=1 order by project_id ASC") or die(mysql_error());
...
function get_washer_service($washer_id , $lang) {

    $data = array();
    $result = mysql_query("SELECT * FROM `services` WHERE `washer_id`='$washer_id' and `display`=1 order by service_id ASC") or die(mysql_error());
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        if ($lang == "ar") {
            $services["service_name"] = $row["service_name_ar"];
        } else {
            $services["service_name"] = $row["service_name_en"];
        }
        $services["service_price"] = $row["service_price"];
        $services["washer_id"] = $row["washer_id"];

        $service_id = $row["service_id"];
        $services["service_price"] = number_format((float) ($service_id), 3, '.', '');
...
function get_washer_images($washer_id) {

    $data = array();
    $result = mysql_query("SELECT * FROM `washer_images` WHERE `washer_id`='$washer_id' order by id DESC ") or die(mysql_error());
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        $images["washer_id"] = $row["washer_id"];
...
function get_washer_address($washer_id , $lang) {

    $data = array();
    $result = mysql_query("SELECT * FROM `washer_address` WHERE `washer_id`='$washer_id' order by id DESC ") or die(mysql_error());
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        $address["id"] = $row["id"];
        if ($lang == "ar") {
            $address["address"] = $row["address_ar"];
        } else {
            $address["address"] = $row["address_en"];
        }
        $address["lat"] = $row["lat"];
        $address["long"] = $row["long"];
        $address["washer_id"] = $row["washer_id"];
...
function get_washer_contact($washer_id) {

    $data = array();
    $result = mysql_query("SELECT * FROM `washer_contact` WHERE `washer_id`='$washer_id' order by id DESC ") or die(mysql_error());
...
function get_washer_work_time($washer_id , $lang) {

    $data = array();
    $result = mysql_query("SELECT * FROM `work_time` WHERE `washer_id`='$washer_id' order by id DESC ") or die(mysql_error());
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        $work_time["id"] = $row["id"];
        if ($lang == "ar") {
            $work_time["day"] = $row["day_ar"];
        } else {
            $work_time["day"] = $row["day_en"];
        }
        $work_time["from"] = $row["from"];
        $work_time["to"] = $row["to"];
