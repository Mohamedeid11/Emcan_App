                    ...
                    <div class="panel">
                        <div class="panel-body">
                            <table class="table table-striped" id="datatable-editable">
                                ...
                                <tr>
                                    <th>#</th>
                                    <th><?=lang('gallery_name_english')?></th>
                                    <th><?=lang('gallery_name_arabic')?></th>
                                    <th><?=lang('image')?></th>
                                    <th><?= lang('status')?></th>
                                    <th><?= lang('date_add')?></th>
                                    <th><?= lang('action')?></th>
                                ...
                                $query = $con->query("SELECT * FROM `gallery` ORDER BY `gallery_id` ASC");
                                $x = 1;
                                while ($row = mysqli_fetch_assoc($query)) {
                                    $gallery_id = $row['gallery_id'];
                                    $gallery_title_en = $row['gallery_title_en'];
                                    $gallery_title_ar = $row['gallery_title_ar'];
                                    $gallery_image = $row['gallery_image'];
                                    $display = $row['display'];
                                    $date = $row['date'];
                                    ?>
                                    <tr class="gradeX<?= $gallery_id ?>">
                                        <td>
                                            <?= $x ?>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="<?= $gallery_image; ?>" class="image-popup" title="<?= $gallery_title_en; ?>">
                                                <img src="<?= $gallery_image; ?>" class="thumb-img" alt="<?= $gallery_title_en; ?>" height="100" style="width:100px;">
                                            </a>
                                        </td>
                                        <td>
                                            <?php if ($display == 1) { ?>
                                                <input class="change_status_off" data-id="<?=$gallery_id; ?>" type="checkbox"
                                                       checked
                                                       data-plugin="switchery" data-color="#81c868"/>
                                                <?php
                                            }else if ($display == 0) {
                                                ?>
                                                <input class="change_status_on" data-id="<?=$gallery_id; ?>" type="checkbox"
                                                       data-plugin="switchery" data-color="#81c868"/>
                                                <?php
                                            }
                                            ?>
                                        ...
                                        <td><?= $date; ?></td>
                                        <td class="actions">
                                            <a href="gallery_edit.php?galleryID=<?= $gallery_id; ?>" class="on-default"><i class="fa fa-pencil"></i></a>
                                        </td>
                                        <td class="actions">
                                            <a href="<?= $gallery_id; ?>" class="on-default remove-row"><i class="fa fa-trash-o"></i></a>
                                        </td>
                                    </tr>
                                    <?php
                                    $x++;
    ...
    $(document).ready(function () {
        $('body').on('click', '.on-default', function () {
            ...
            $("#dialogConfirm").click(function () {
                var dataString = 'gallery_id=' + gallery_id;
                $.ajax({
                    type: "POST",
